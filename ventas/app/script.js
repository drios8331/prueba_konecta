...
function verificarCantidad() {
  ...
  if (arrayVent.length <= 0) {
    ...
    newStock =
      stock - (parseInt(cant) + parseInt(arrayVent[0].cantidadProducto));
  }

  if (newStock < 0) {
    mostrarError(
      `No hay suficiente Stock en inventario, hay ${stock} unidades actualmente.`,
      true,
      "bg-danger"
    );
    producto.value = "";
...
function agregarVentas() {
  ...
  if (arrayVenta.length > 0) {
    ...
    if (filtroCoincidencias.length > 0) {
      ...
      const cantidadVentaAnterior = filtroCoincidencias[0].cantidadProducto;
      const idProductoAnterior = filtroCoincidencias[0].idProducto;
      const codigoProductoAnterior = filtroCoincidencias[0].codigoProducto;
      const nombreProductoAnterior = filtroCoincidencias[0].nombreProducto;
      const precioProductoAnterior = filtroCoincidencias[0].precioProducto;
      const cantidadTotalProducto = cantidadVentaActual + cantidadVentaAnterior;
...
function precioTotal(arrayVenta) {
  ...
  let totalArray = arrayTotal.reduce(
    (total, valorTotal) => total + valorTotal.precioTotalUnitario, 0
  );
  let totalAPagar = totalArray;
