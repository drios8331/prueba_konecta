...
class Modal
{
    ...
    public function modalTable($color, $tituloModal, $contenido)
    {
        ...
        echo "<script>$('#modalAlerta').modal('show')</script>";
        echo "<script>$('#close').click(function(){location.reload()});</script>";
        echo  "<script>$('#tableFactura').DataTable({ responsive: true, order: [[0, 'desc']] });</script>";
    }
    ...
    public function modalImprimir($color, $tituloModal, $contenido)
    {
        $modal = "<!-- Modal -->";
        $modal .= "<div class='modal fade' id='modalImprimir' data-bs-backdrop='static' data-bs-keyboard='false' tabindex='-1' aria-labelledby='staticBackdropLabel' aria-hidden='true'>";
        $modal .= "  <div id='printThis' class='modal-dialog modal-dialog-centered'>";
        $modal .= "    <div class='modal-content'>";
        $modal .= "      <div class='modal-header  border-0'>";
        $modal .= "        <h5 class='modal-title $color' id='exampleModalLabel'>" . $tituloModal . "</h5>";
        $modal .= "                <button type='button' class='btn-close' id='close' data-bs-dismiss='modal' aria-label='Close'></button>";
        ...
        $modal .= "  </div>";
        $modal .= "</div>";
        $modal .= "<script>$('#modalImprimir').modal('show')</script>";
        $modal .= "<script>$('#close').click(function(){location.reload()});</script>";
        $modal .= "<script>imprimirModal('modalImprimir');</script>";
        return $modal;
    }
